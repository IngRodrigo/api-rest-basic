...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($data != null) {
            ...
            if ($conexion != null) {
                        $query = "SELECT
                                    ...
                                    u.update_at AS 'actualizado',
                                    CASE
                                WHEN u.active = 'Y' THEN
                                    '1'
                                ELSE
                                    '0'
                                END AS 'id_estado',
                                ...
                                END AS 'estado',
                                 ...
                                 g.description AS 'rol'

                                FROM
                                    sec_users AS u
                                INNER JOIN sec_users_groups AS ug ON u.login = ug.login
                                INNER JOIN sec_groups AS g ON ug.group_id = g.group_id
                                where u.login='$usuario' and u.pswd='$password'";
    ...
    if ($data['accion'] == 'crear_usuario') {
        ...
        if ($conexion != null) {
            ...
            $query = "INSERT INTO sec_users (login, pswd, name, active, apellido, documento, create_at, update_at, cel) 
				values
				 ('$userName', 
				 '$userPassword', 
				 '$nombre', 
    ...
    if ($data['accion'] == 'insertar_devolucion') {
        ...
        if ($status == 'ok') {
            ...
            foreach ($detalles as $key => $detalle) {
                ...
                $insertDetalle = "INSERT INTO dtl_devoluciones (KCOO, DCTO, DOCO, LNID, AN8, AITM, UORG, LPRC, UOM, i55DEPR, DRQJ, UPC3, s55PROMFM, LOCN, create_at, update_at, id_user, cod_articulo, descripcion) 
								values (
								'$KCOO', '$DCTO', '$DOCO', '$LNID', '$AN8', '$AITM', '$UORG', '$LPRC', '$UOM', '$i55DEPR', '$DRQJ', '$UPC3', '$s55PROMFM', '$LOCN',
								'$create_at','$update_at', '$id_user', '$cod_articulo', '$descripcion')";
